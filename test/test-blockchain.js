...
async function createTransaction(name, password, toAddress, amount) {
  ...
  try {
    ...
    try {
      ...
      const loadWalletResult = await callApi('/wallet/load', 'POST', { name, password });
  ...
  rl.question('输入钱包名称: ', (name) => {
    rl.question('输入钱包密码: ', async (password) => {
      try {
        ...
        const files = fs.existsSync(WALLET_DIR) ? fs.readdirSync(WALLET_DIR) : [];
        console.log('钱包目录中的文件:', files);
        
        const loadResult = await callApi('/wallet/load', 'POST', { name, password });
...
function showMenu() {
  ...
  console.log('7. 列出所有钱包');
  console.log('8. 测试钱包加载');
  console.log('9. 退出');
